...
large majority of their duration. If the mock server runs as a daemon *thread*
in the pytest process, the GIL starvation causes HTTP timeouts.

Running the server in its own process gives it an independent GIL.

Implementation: uses ``subprocess.Popen`` instead of ``multiprocessing.spawn``
because macOS Apple Silicon GitHub runners hang during spawn (botocore's service
model loading triggers Gatekeeper scanning in the fresh spawn context).
"""
...
class MockServerProcess:
    ...
    def start(self) -> "MockServerProcess":
        ...
        if not line or not line.startswith("http"):
            rc = self._proc.poll()
            raise RuntimeError(
                f"Mock server failed to start (got: {line!r}, exitcode={rc})"
            )
        self.base_url = line
