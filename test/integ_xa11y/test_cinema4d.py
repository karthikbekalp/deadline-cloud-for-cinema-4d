...
def _close_explorer_window(history_dir: Path) -> None:
    ...
    # Always dump the live app/window tree to a file so we can see how Explorer
    # actually appears in the UIA tree -- this survives pytest's stdout capture
    # on passing runs (unlike log()/print). See _dump_window_tree.
    _dump_window_tree("before Explorer close", history_dir)
    ...
    # Nothing matched -- dump again so the file shows the final state too.
    _dump_window_tree("after Explorer close failed to match", history_dir)


def _explorer_diag_path() -> Path:
    """Where the Explorer diagnostics file is written. Overridable via
    DEADLINE_CLOUD_EXPLORER_DIAG; defaults to a stable temp-dir path."""
    override = os.environ.get("DEADLINE_CLOUD_EXPLORER_DIAG")
    if override:
        return Path(override)
    return Path(tempfile.gettempdir()) / "c4d_explorer_diag.log"


def _dump_window_tree(reason: str, history_dir: Path) -> None:
    """Append a snapshot of every app and its top-level windows to a file.

    Written to disk (not just logged) so it survives pytest's stdout capture on
    passing runs -- the user can hand us this file to see exactly how Explorer
    shows up on Windows (which app hosts it, the window's role/name/actions, and
    -- for any window whose name looks bundle-related -- a shallow subtree so we
    can spot a title-bar Close button). Best-effort, never raises.

    Captures for each app: name, pid, and the app element's own actions; for
    each top-level window: role, name, actions; and for windows whose name
    mentions the bundle leaf or "DeadlineCloud" (the likely Explorer window), a
    depth-2 dump so child controls (e.g. a Close button) are visible.
    """
    path = _explorer_diag_path()
    leaf = history_dir.name
    lines = [
        f"===== window tree: {reason} =====",
        f"history_dir={history_dir}  leaf={leaf!r}",
    ]
    try:
        for app in xa11y.App.list():
            try:
                lines.append(f"APP name={app.name!r} pid={app.pid}")
                for w in app.children():
                    role = getattr(w, "role", None)
                    name = getattr(w, "name", None) or ""
                    actions = getattr(w, "actions", None)
                    lines.append(f"    WINDOW role={role!r} name={name!r} actions={actions!r}")
                    # If this window looks like our Explorer window, dump a
                    # shallow subtree so we can see how to close it.
                    looks_related = (leaf and leaf in name) or ("DeadlineCloud" in name)
                    if looks_related:
                        try:
                            lines.append("      --- subtree (depth=2) ---")
                            lines.append(w.dump(max_depth=2))
                        except Exception as e:
                            lines.append(f"      <subtree dump failed: {e!r}>")
            except Exception as e:
                lines.append(f"    <error enumerating app {app.name!r}: {e!r}>")
    except Exception as e:
        lines.append(f"<App.list() failed: {e!r}>")
    text = "\n".join(lines)
    try:
        with open(path, "a", encoding="utf-8") as f:
            f.write(text + "\n\n")
        # Print the path prominently so it's easy to find and hand back to us.
        log(f"[EXPLORER-DIAG] window-tree diagnostics written to: {path}")
    except Exception as e:
        log(f"could not write window-tree diagnostics to {path}: {e!r}")
    # Also print (visible when run with -s), so both channels are covered.
    print(text, flush=True)


def _explorer_diag(msg: str) -> None:
    """Log to both the test log and the persistent Explorer diagnostics file,
    so close attempts are visible on passing Windows runs."""
    log(msg)
    try:
        with open(_explorer_diag_path(), "a", encoding="utf-8") as f:
            f.write(msg + "\n")
    except Exception:
        pass
...
def _try_close_window(window, title: str) -> bool:
    ...
    Logs which strategy ran (and why each failed) to the diagnostics file."""
    try:
        available = list(window.actions or [])
    except Exception as e:
        available = []
        _explorer_diag(f"could not read window.actions: {e!r}")
    _explorer_diag(f"attempting to close window {title!r}; actions={available!r}")
    for action in ("close", "invoke"):
        try:
            if action in available:
                window.perform_action(action)
                _explorer_diag(f"closed Explorer window via {action!r} action: {title!r}")
                return True
        except Exception as e:
            _explorer_diag(f"window {action!r} action failed (trying next): {e!r}")
    ...
    try:
        ...
        _explorer_diag(f"closed Explorer window via Ctrl+W: {title!r}")
        return True
    except Exception as e:
        _explorer_diag(f"Ctrl+W close failed: {e!r}")
    return False
